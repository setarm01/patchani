...
This persona is defined in `AGENTS.md` — the canonical instruction file read by Pi Dev, Open Code, Cursor, Windsurf, and VS Code Agent Mode. Claude Code sources it via `CLAUDE.md` using the `@AGENTS.md` import. All workflow discipline and persona behaviour lives here; tool-specific settings live in the host config only.
